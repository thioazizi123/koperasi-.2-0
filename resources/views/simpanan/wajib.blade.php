...
@section('content')
    ...
    <div style="margin-top: 2rem; background: white; padding: 2rem; border-radius: 1.5rem; box-shadow: var(--shadow);">
        ...
        <form action="{{ route('simpanan.wajib.store') }}" method="POST" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1.5rem; align-items: end;">
            ...
            <div>
                <label style="display: block; margin-bottom: 0.5rem; font-size: 0.875rem; color: #64748b;">Jumlah (Min. Rp 20.000)</label>
                <input type="number" name="amount" value="20000" min="20000" required style="width: 100%; padding: 0.75rem; border: 1px solid #e2e8f0; border-radius: 0.5rem; outline: none;">
            </div>
            <div>
    ...
    <div style="margin-top: 2.5rem; background: white; padding: 2rem; border-radius: 1.5rem; box-shadow: var(--shadow); overflow-x: auto;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
            <h2 style="font-size: 1.25rem;">Laporan Simpanan Wajib Tahun {{ $year }}</h2>
            <form action="{{ route('simpanan.wajib') }}" method="GET" style="display: flex; gap: 0.5rem; align-items: center;">
            ...
            <tbody>
                @foreach($members as $index => $member)
                    <tr style="border-bottom: 1px solid #f1f5f9;">
                        ...
                        <td style="padding: 0.75rem; border: 1px solid #e2e8f0; font-family: monospace;">{{ $member->member_no ?? '-' }}</td>
