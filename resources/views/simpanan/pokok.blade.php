    ...
    <table style="width: 100%; border-collapse: collapse;">
            ...
            @forelse($members as $member)
                ...
                <tr style="border-bottom: 1px solid #f1f5f9;">
                    ...
                    <td style="padding: 1rem 0;">
                        @if($isPaid)
                            <span style="background: #dcfce7; color: #166534; padding: 0.25rem 0.75rem; border-radius: 1rem; font-size: 0.875rem;">Lunas</span>
                        @else
                            <span style="background: #fee2e2; color: #991b1b; padding: 0.25rem 0.75rem; border-radius: 1rem; font-size: 0.875rem;">Belum Lunas</span>
                        @endif
                    </td>
                    <td style="padding: 1rem 0; text-align: right;">
                        @if(!$isPaid)
                            <form action="{{ route('simpanan.pokok.store') }}" method="POST" style="display: inline;">
                                ...
                                <button type="submit" style="background: var(--primary-color); color: white; padding: 0.5rem 1rem; border: none; border-radius: 0.5rem; cursor: pointer; font-size: 0.875rem;">
                                    Bayar Rp 50.000
                                </button>
                            </form>
                        @else
                            <form action="{{ route('simpanan.pokok.destroy', $member->id) }}" method="POST" style="display: inline;" onsubmit="return confirm('Apakah Anda yakin ingin membatalkan status lunas ini?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit" style="background: #f1f5f9; color: #64748b; padding: 0.5rem 1rem; border: none; border-radius: 0.5rem; cursor: pointer; font-size: 0.875rem;">
                                    Batalkan
                                </button>
                            </form>
                        @endif
        ...
        <tfoot>
            <tr style="font-weight: bold; background: #f8fafc;">
                ...
                <td colspan="2" style="padding: 1rem; text-align: right; border-top: 2px solid #e2e8f0; font-size: 1.125rem; color: var(--primary-color);">
